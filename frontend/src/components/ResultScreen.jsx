...
export default function ResultScreen({ inviteVideo, parentsName, venue, onReset }) {
  ...
  useEffect(() => {
    ...
    getUserRegion(venue)
      .then((region) => {
        ...
        setPricingReady(Boolean(region && region.amount > 0));
      })
      .catch(() => {
        if (!mounted) return;
        setUserRegion(null);
  ...
  }, [inviteVideo, parentsName, isImage]);
  ...
  const handleDownload = useCallback(() => {
    if (paymentRequired && !pricingReady) {
      return;
    }
    if (requiresPayment) {
      handlePurchase();
  ...
  }, [paymentRequired, pricingReady, requiresPayment, handlePurchase, handleDownloadInternal]);
  ...
  return (
    <div className="result-screen">
        ...
        {mediaUrl ? (
          isImage ? (
            <img 
              ...
              alt="baby shower invite preview"
            />
          ) : (
            <video
              src={mediaUrl}
      ...
      <div className="action-buttons">
        ...
        <button
          className={requiresPayment ? "buy-download-btn" : "download-btn"}
          onClick={handleDownload}
          disabled={isPurchasing || (paymentRequired && !pricingReady)}
        >
          {isPurchasing ? (
            <>⏳ processing...</>
          ) : paymentRequired && !pricingReady ? (
            <>⏳ loading price...</>
          ) : requiresPayment ? (
            ...
            <>
              ...
              {isImage ? 'download image' : 'download video'}
</>
          )}
        </button>
